var Controller = ['$scope', "$document", "$element", function ($scope, $document, $element) 
{
	...
	$scope.selectSubRecord = function(idx){
		...
		var Alias = getAlias($scope.layout.qHyperCube.qDimensionInfo, $scope.layout.settings.Campo);
		console.log("Alias: "+Alias);
		console.log(this.row.SubRec[idx]);

		if($scope.layout.settings.selezionabile) {
			if(isNumeric(this.row.SubRec[idx][$scope.layout.settings.Campo].Content))
				$scope.QlikApp.field(Alias).selectValues([parseInt(this.row.SubRec[idx][$scope.layout.settings.Campo].Content)], false);
			else
				$scope.QlikApp.field(Alias).selectValues([this.row.SubRec[idx][$scope.layout.settings.Campo].Content], false);
